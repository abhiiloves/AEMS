...
break the app outright, now fixed in `014_fix_rls_recursion.sql` and
`lib/auth/session.ts` / `middleware.ts`:

1. **RLS infinite recursion.** `current_app_user()`/`current_app_role()`
   queried the `users` table, which itself has RLS calling those same
   functions to evaluate its own policy -- infinite recursion on
   essentially every query in the app. Fixed by marking the lookup
   functions `SECURITY DEFINER` so they bypass RLS internally (they
   still only ever return data about the calling user, so this doesn't
   widen any policy's actual permission).
2. **`audit_logs` had no INSERT policy.** Every route in the app
   inserts audit rows directly through the RLS-enforced client, but the
   original policy file only had a comment saying inserts would go
   through a security-definer function that was never actually built --
   so every audit-log write in the whole system was being silently
   rejected. Added a real insert policy (`with check` still stops
   forging another user's identity in the log).
3. **Session tracking was tied to Supabase's rotating access token.**
   `sessions.session_token` was being matched against
   `session.access_token`, which `@supabase/ssr` silently refreshes --
   so a perfectly valid session could start failing the idle-timeout
   check the moment the token rotated. Fixed by minting our own opaque
   session id at login (`lib/auth/session.ts`), carried in its own
   `aems_sid` cookie, independent of the auth token's lifecycle.

Also added in this pass: the real PG logo/branding assets and the
device-preview images, copied from the previous system's `public/`
folder (`public/pg-logo.png`, `public/logo.png`,
`public/device-previews/*.jpg`) and wired into the login page and
sidebar -- the login screen now matches the previous system's actual
`LoginScreen.tsx` (white rounded card, gray gradient background, blue
"Request OTP" / emerald "Verify & Login" steps) instead of the
navy-card version built earlier in this project, per request.

**If you're merging this with a separately-edited copy** (e.g. one with
a `014_department_hierarchy.sql` migration adding
department/sub-department scoping): keep that migration, but make sure
`014_fix_rls_recursion.sql` and the session-cookie changes above are
also present -- they aren't optional, the app does not work correctly
